# Autogenic Training — Explainer

**How the method works and how to practise it · ~4 minutes**

<!-- render: --speed 0.9 --stretch 1.0 --lead-in 0.5 -->

Listen to this once, before your first practice. It carries the teaching, so
that the practice tracks don't have to — a session that stops to explain itself
breaks the very state it is trying to produce.

---

## Production notes

This is the one track that is **not** relaxation audio. It is delivered at
normal conversational pace, with short pauses for structure rather than dwell
time. The render settings above are declared in this file so a later re-render
can't silently slow it to session pace.

- Voice: clear, direct, unhurried but not soft. Explaining, not soothing.
- Pauses of 1–2 seconds mark topic changes. Nothing longer.
- `--speed 0.9` measures 163 wpm. The voice's natural rate reads 173 here,
  which is quick for a track carrying dose figures and medical cautions; the
  relaxation sessions run 128–139 for contrast.

---

## Narration

<!-- narration:start -->

This is a short explanation of what you're about to practise, and how to make
it work. About four minutes. You don't need to relax for this part — just
listen normally.

[pause 1.5]

Autogenic training was developed in Berlin in the nineteen twenties by a doctor
called Johannes Schultz. "Autogenic" means generated from within, and that is
the whole idea. You are not being relaxed by me, or by a recording. You are
learning to produce the state yourself.

[pause 1.5]

There are six standard exercises. The first, and the only one you should touch
for now, is heaviness in the arms.

[pause 1.5]

Here is why heaviness. When your body shifts out of alert mode, your muscles
release the low background tension they hold all day. Heaviness is what that
release feels like from the inside. So the sensation isn't really the goal —
it's the signal that the shift has happened.

[pause 1.5]

What you'll actually do is repeat one short phrase: my right arm is heavy.

[pause 1]

Say it silently, inside your own head, in your own voice. Not aloud. Four to
six times, slowly, and then let it go.

[pause 1.5]

Now the part that catches everyone.

[pause 1]

You cannot make this happen by trying. Effort is the one thing that reliably
blocks it. If you concentrate hard on your arm and demand that it feel heavy,
it won't.

[pause 1]

It's like falling asleep. You can arrange the conditions, but you can't force
the moment. So hold the phrase lightly, notice whatever is actually there, and
let it be uninteresting. This is called passive concentration, and it's the
whole skill.

[pause 2]

What should you expect? Probably nothing at all for the first few days. That's
normal, and it isn't failure.

[pause 1]

Heaviness usually appears somewhere between a few days and two weeks of regular
practice. It may not arrive as heaviness exactly — people describe warmth,
tingling, sinking, or the arm feeling larger, or further away than it is. Any
of those count.

[pause 1.5]

Sometimes you'll get the opposite: restlessness, a twitch, an urge to move, or
unexpected emotion. That's known as an autogenic discharge. It's a recognised
part of the process, not a sign that something is wrong. If it's uncomfortable,
open your eyes, move your arms and legs, and stop there for that session.

[pause 2]

How often. This matters more than anything else I've said.

[pause 1]

Short and frequent beats long and occasional. Aim for about ninety seconds,
three times a day — morning, midday, and evening. Frequency is what builds the
response. One long session a week will not, however diligently you sit through
it.

[pause 2]

Every practice ends the same way, and this part is not optional.

[pause 1]

Bend and stretch your arms firmly, fists clenched, several times. Take two deep
breaths. Then open your eyes. This returns your muscle tone and circulation to
normal. Skip it and you can be left groggy or heavy-headed for a while
afterwards.

[pause 1]

The one exception is if you're practising in bed to fall asleep. In that case,
don't do it — just stay lying down and let yourself drift off.

[pause 2]

Two cautions before you start. Don't practise while driving, or doing anything
else that needs your attention. And if you have low blood pressure, a heart
condition, diabetes, or epilepsy, speak to your doctor before making this a
daily habit — it genuinely does lower your heart rate and blood pressure.

[pause 2]

Finally, how to use the other recordings.

[pause 1]

There's a short session, about seven minutes, that goes through a complete
practice with you. Use that one to learn the shape of it. There's a longer,
fuller version as well, if you ever want it.

[pause 1]

But neither of them is the destination. Notice that everything I've described
is something you do, not something a recording does to you. The aim is that
within a couple of weeks you're practising without either — ninety seconds, on
a train or at your desk, with nothing but your own attention.

[pause 2]

That's everything. Start with the example, then practise on your own.

[pause 1]

Little, and often.

<!-- narration:end -->
